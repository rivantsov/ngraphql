...
namespace NGraphQL.Server.Parsing {
  ...
  public partial class RequestBuilder {
   ...
   // nested class TopRequestItems
    ...
    private IList<VariableDecl> BuildOperationVariables(Node varDefsNode) {
      if(varDefsNode == null)
        return VariableDecl.EmptyList;

      var varList = new List<VariableDecl>();
      foreach(var vn in varDefsNode.ChildNodes) {
        ...
        var varDef = new VariableDecl() { Name = name, TypeRef = typeRef, Location = vn.GetLocation() };
        // check default value
